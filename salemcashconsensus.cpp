...
inline int set_error(salemcashconsensus_error* ret, salemcashconsensus_error*consensus_error serror)
{
    if (ret)
        *ret = serror;
    return 0;
...
{
    return (flags & ~(salemcashconsensus_SCRIPT_FLAGS_VERIFY_ALL)) == 0;
}
                                    ...
                                    unsigned int nIn, unsigned int flags, salemcashconsensus_error* err)
{
    if (!verify_flags(flags)) {
        return salemcashconsensus_ERR_INVALID_FLAGS;
    }
    try {
...
int salemcashconsensus_verify_script_with_amount(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen, int64_t amount,
                                    const unsigned char *txTo        , unsigned int txToLen,
                                    unsigned int nIn, unsigned int flags, salemcashconsensus_error* err)
{
    CAmount am(amount);
...
int salemcashconsensus_verify_script(const unsigned char *scriptPubKey, unsigned int scriptPubKeyLen,
                                   const unsigned char *txTo        , unsigned int txToLen,
                                   unsigned int nIn, unsigned int flags, salemcashconsensus_error* err)
{
...
unsigned int salemcashconsensus_version()
{
    // Just use the API version for now
    return SALEMCASHCONSENSUS_API_VER;
}
