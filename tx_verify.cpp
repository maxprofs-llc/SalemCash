...
unsigned int GetP2SHSigOpCount(const CTransaction& tx, const CCashViewCache& inputs)
{
    ...
    {
        const Cash& cash = inputs.AccessCash(tx.vin[i].prevout);
        assert(!cash.IsSpent());
...
bool Consensus::CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCashViewCache& inputs, int nSpendHeight, CAmount& txfee)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        nValueIn += cash.out.nValue;
        if (!MoneyRange(cash.out.nValue) || !MoneyRange(nValueIn)) {
