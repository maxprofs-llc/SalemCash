    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return SalemcashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, SalemcashUnits::separatorNever);
            else
                return SalemcashUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+SalemcashUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
...
void RecentRequestsTableModel::addNewRequest(const SendCashRecipient &recipient)
{
