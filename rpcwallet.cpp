...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nReturns a new SalemCash address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"address\"    (string) The new SalemCash address\n"
            "\nExamples:\n"
...
UniValue getlabeladdress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current SalemCash address for receiving payments to this label.\n"
            "\nArguments:\n"
            ...
            "\nResult:\n"
            "\"address\"          (string) The label SalemCash address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new SalemCash address, for receiving change.\n"
            "This is for use with raw transactions, NOT normal use.\n"
            "\nArguments:\n"
            "1. \"address_type\"           (string, optional) The address type to use. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\". Default is set by -changetype.\n"
            "\nResult:\n"
            "\"address\"    (string) The SalemCash address\n"
            "\nExamples:\n"
...
UniValue setlabel(const JSONRPCRequest& request)
{
    ...
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SalemCash address");
    }
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The SalemCash address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a SalemCash address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static CTransactionRef SendMoney(CWallet * const pwallet, const CTxDestination &address, CAmount nValue, bool fSubtractFeeFromAmount, const CCashControl& cash_control, mapValue_t mapValue, std::string fromAccount)
{
    ...
    CTransactionRef tx;
    if (!pwallet->CreateTransaction(vecSend, tx, reservekey, nFeeRequired, nChangePosRet, strError, cash_control)) {
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The SalemCash address to send to.\n"
    ...
    if (!request.params[5].isNull()) {
        cash_control.signalRbf = request.params[5].get_bool();
    }

    if (!request.params[6].isNull()) {
        cash_control.m_confirm_target = ParseConfirmTarget(request.params[6]);
    }

    if (!request.params[7].isNull()) {
        if (!FeeModeFromString(request.params[7].get_str(), cash_control.m_fee_mode)) {
    ...
    CTransactionRef tx = SendMoney(pwallet, dest, nAmount, fSubtractFeeFromAmount, cash_control, std::move(mapValue), {} /* fromAccount */);
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw std::runtime_error(
            ...
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The SalemCash address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The SalemCash address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a SalemCash address."
            + HelpRequiringPassphrase(pwallet) + "\n"
            "\nArguments:\n"
    ...
    CCashControl no_cash_control; // This is a deprecated API
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The SalemCash address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            "                           Those recipients will receive less SalemCash amount than you enter in their corresponding amount field.\n"
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SalemCash address: ") + name_);
        }
    ...
    CTransactionRef tx;
    bool fCreated = pwallet->CreateTransaction(vecSend, tx, keyChange, nFeeRequired, nChangePosRet, strFailReason, cash_control);
    if (!fCreated)
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
        std::string msg = "addmultisigaddress nrequired [\"key\",...] ( \"label\" \"address_type\" )\n"
            ...
            "Each key is a SalemCash address or hex-encoded public key.\n"
            ...
            "2. \"keys\"                         (string, required) A json array of SalemCash addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"                  (string) SalemCash address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
...
UniValue addwitnessaddress(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "addwitnessaddress is deprecated and will be fully removed in v0.17. "
            "To use addwitnessaddress in v0.16, restart salemcashd with -deprecatedrpc=addwitnessaddress.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The SalemCash address of the transaction. Not present for \n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"address\",    (string) The SalemCash address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue gettransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "      \"address\" : \"address\",          (string) The SalemCash address involved in the transaction\n"
...
UniValue walletpassphrase(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending SalemCash\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending SalemCash\n"
...
UniValue lockunspent(const JSONRPCRequest& request)
{
    ...
    for (const COutPoint& outpt : outputs) {
        if (fUnlock) pwallet->UnlockCash(outpt);
        else pwallet->LockCash(outpt);
    }
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw std::runtime_error(
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the SalemCash address\n"
    ...
    if (!request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid SalemCash address: ") + input.get_str());
            }
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The SalemCash address to receive the change\n"
    ...
    if (!request.params[1].isNull()) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid SalemCash address");
            }

            cashControl.destChange = dest;
        }
        ...
        if (options.exists("change_type")) {
            ...
            cashControl.m_change_type = ParseOutputType(options["change_type"].get_str(), pwallet->m_default_change_type);
            if (cashControl.m_change_type == OutputType::NONE) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, strprintf("Unknown change type '%s'", options["change_type"].get_str()));
            }
        }

        if (options.exists("includeWatching"))
            cashControl.fAllowWatchOnly = options["includeWatching"].get_bool();
        ...
        {
            cashControl.m_feerate = CFeeRate(AmountFromValue(options["feeRate"]));
            cashControl.fOverrideFeeRate = true;
        }
        ...
        if (options.exists("replaceable")) {
            cashControl.signalRbf = options["replaceable"].get_bool();
        }
        if (options.exists("conf_target")) {
            if (options.exists("feeRate")) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Cannot specify both conf_target and feeRate");
            }
            cashControl.m_confirm_target = ParseConfirmTarget(options["conf_target"]);
        }
    ...
    if (!pwallet->FundTransaction(tx, nFeeOut, changePosition, strFailReason, lockUnspents, setSubtractFeeFromOutputs, cashControl)) {
        throw JSONRPCError(RPC_WALLET_ERROR, strFailReason);
    }
...
UniValue bumpfee(const JSONRPCRequest& request)
{
    ...
    if (!request.params[1].isNull()) {
        ...
        if (options.exists("confTarget") && options.exists("totalFee")) {
            ...
            cash_control.m_confirm_target = ParseConfirmTarget(options["confTarget"]);
        ...
        if (options.exists("replaceable")) {
            cash_control.signalRbf = options["replaceable"].get_bool();
        }
        if (options.exists("estimate_mode")) {
            if (!FeeModeFromString(options["estimate_mode"].get_str(), cash_control.m_fee_mode)) {
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid estimate_mode parameter");
            }
        }
    }
    ...
    CAmount new_fee;
    CMutableTransaction mtx;
    feebumper::Result res = feebumper::CreateTransaction(pwallet, hash, cash_control, totalFee, errors, old_fee, new_fee, mtx);
...
UniValue generate(const JSONRPCRequest& request)
{
    ...
    std::shared_ptr<CReserveScript> cashbase_script;
    pwallet->GetScriptForMining(cashbase_script);
    ...
    if (cashbase_script->reserveScript.empty()) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "No cashbase script available");
    }

    return generateBlocks(cashbase_script, num_generate, max_tries, true);
}
...
UniValue getaddressinfo(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
            ...
            "\nReturn information about the given SalemCash address. Some information requires the address\n"
            "to be in the wallet.\n"
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The SalemCash address to get the information of.\n"
            "\nResult:\n"
            "{\n"
            "  \"address\" : \"address\",        (string) The SalemCash address validated\n"
