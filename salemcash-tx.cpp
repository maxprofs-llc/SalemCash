...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") || gArgs.IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s salemcash-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  salemcash-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded salemcash transaction") + "\n" +
              "  salemcash-tx [options] -create [commands]   " + _("Create hex-encoded salemcash transaction") + "\n" +
              "\n";
        ...
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
...

            {
                ...
                if (prevOut.exists("amount")) {
                    newcash.out.nValue = AmountFromValue(prevOut["amount"]);
                }
                newcash.nHeight = 1;
