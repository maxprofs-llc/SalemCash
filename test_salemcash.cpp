...
TestingSetup::TestingSetup(const std::string& chainName) : BasicTestingSetup(chainName)
{
    const CChainParams& chainparams = Params();
        ...
        ClearDatadirCache();
        pathTemp = fs::temp_directory_path() / strprintf("test_salemcash_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
        ...
        pcashdbview.reset(new CCashViewDB(1 << 23, true));
        pcashTip.reset(new CCashViewCache(pcashdbview.get()));
...
TestChain100Setup::TestChain100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    ...
    for (int i = 0; i < COINBASE_MATURITY; i++)
    {
        ...
        cashbaseTxns.push_back(*b.vtx[0]);
    }
}
...
TestChain100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& txns, const CScript& scriptPubKey)
{
    ...
    // Replace mempool-selected txns with just cashbase plus passed-in txns:
    block.vtx.resize(1);
