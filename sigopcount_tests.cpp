...
BOOST_AUTO_TEST_CASE(GetTxSigOpCost)
{
    ...
    // Multisig script (legacy counting)
    {
        ...
        BuildTxs(spendingTx, cash, creationTx, scriptPubKey, scriptSig, CScriptWitness());
        // Legacy counting only includes signature operations in scriptSigs and scriptPubKeys
        // of a transaction and does not take the actual executed sig operations into account.
        // spendingTx in itself does not contain a signature operation.
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags) == 0);
        // creationTx contains two signature operations in its scriptPubKey, but legacy counting
        // is not accurate.
        assert(GetTransactionSigOpCost(CTransaction(creationTx), cash, flags) == MAX_PUBKEYS_PER_MULTISIG * WITNESS_SCALE_FACTOR);
    ...
    // Multisig nested in P2SH
    {
        ...
        BuildTxs(spendingTx, cash, creationTx, scriptPubKey, scriptSig, CScriptWitness());
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags) == 2 * WITNESS_SCALE_FACTOR);
        assert(VerifyWithFlag(creationTx, spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
    ...
    // P2WPKH witness program
    {
        ...
        BuildTxs(spendingTx, cash, creationTx, scriptPubKey, scriptSig, scriptWitness);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags) == 1);
        // No signature operations if we don't verify the witness.
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags & ~SCRIPT_VERIFY_WITNESS) == 0);
        ...
        // The witness of a cashbase transaction is not taken into account.
    ...
    // P2WSH witness program
    {
        ...
        BuildTxs(spendingTx, cash, creationTx, scriptPubKey, scriptSig, scriptWitness);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags) == 2);
        assert(GetTransactionSigOpCost(CTransaction(spendingTx), cash, flags & ~SCRIPT_VERIFY_WITNESS) == 0);
        assert(VerifyWithFlag(creationTx, spendingTx, flags) == SCRIPT_ERR_CHECKMULTISIGVERIFY);
    }
