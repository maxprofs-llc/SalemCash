...
#ifndef SALEMCASH_WALLET_WALLET_H
#define SALEMCASH_WALLET_WALLET_H
...
struct CashSelectionParams
{
    ...
    CashSelectionParams(bool use_bnb, size_t change_output_size, size_t change_spend_size, CFeeRate effective_fee, size_t tx_noinputs_size) : use_bnb(use_bnb), change_output_size(change_output_size), change_spend_size(change_spend_size), effective_fee(effective_fee), tx_noinputs_size(tx_noinputs_size) {}
    CashSelectionParams() {}
};

struct CashEligibilityFilter
{
    const int conf_mine;
    const int conf_theirs;
    const uint64_t max_ancestors;

    CashEligibilityFilter(int conf_mine, int conf_theirs, uint64_t max_ancestors) : conf_mine(conf_mine), conf_theirs(conf_theirs), max_ancestors(max_ancestors) {}
};
...
public:
    ...
    bool SelectCash(const std::vector<COutput>& vAvailableCash, const CAmount& nTargetValue, std::set<CInputCash>& setCashsRet, CAmount& nValueRet,
                    const CCashControl& cash_control, CashSelectionParams& cash_selection_params, bool& bnb_used) const;
    ...
    std::set<COutPoint> setLockedCash;
    ...
    /**
     * populate vCash with vector of available COutputs.
     */
    void AvailableCash(std::vector<COutput>& vCash, bool fOnlySafe=true, const CCashControl *cashControl = nullptr, const CAmount& nMinimumAmount = 1, const CAmount& nMaximumAmount = MAX_MONEY, const CAmount& nMinimumSumAmount = MAX_MONEY, const uint64_t nMaximumCount = 0, const int nMinDepth = 0, const int nMaxDepth = 9999999) const;

    /**
     * Return list of the available cash and the locked cash grouped by non-change output address.
     */
    std::map<CTxDestination, std::vector<COutput>> ListCash() const;
    ...
    /**
     * Shuffle and select cash until nTargetValue is reached while avoiding
     * small change; This method is stochastic for some inputs and upon
     * completion the cash set and corresponding actual target value is
     * assembled
     */
    ...
    void UnlockAllCash();
    void ListLockedCash(std::vector<COutPoint>& vOutpts) const;
    ...
    CAmount GetAvailableBalance(const CCashControl* cashControl = nullptr) const;
     ...
     */
    bool FundTransaction(CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, std::string& strFailReason, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCashControl);
    bool SignTransaction(CMutableTransaction& tx);

    /**
     * Create a new transaction paying the recipients with a set of cash
     * selected by the SelectCash(); Also create the change output, when needed
    ...
    bool CreateTransaction(const std::vector<CRecipient>& vecSend, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosInOut,
                           std::string& strFailReason, const CCashControl& cash_control, bool sign = true);
...
#endif // SALEMCASH_WALLET_WALLET_H
