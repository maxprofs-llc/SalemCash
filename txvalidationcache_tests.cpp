...
BOOST_FIXTURE_TEST_CASE(tx_mempool_block_doublespend, TestChain100Setup)
{
    ...
    // Create a double-spend of mature cashbase txn:
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        spends[i].vin[0].prevout.hash = cashbaseTxns[0].GetHash();
        ...
        BOOST_CHECK(cashbaseKey.Sign(hash, vchSig));
...
// Run CheckInputs (using pcashTip) on the given transaction, for all script
...
void ValidateCheckInputsForAllFlags(CMutableTransaction &tx, uint32_t failing_flags, bool add_to_cache)
{
    ...
    for (uint32_t test_flags=0; test_flags < (1U << 16); test_flags += 1) {
        ...
        bool ret = CheckInputs(tx, state, pcashsTip.get(), true, test_flags, true, add_to_cache, txdata, nullptr);
        ...
        if (ret && add_to_cache) {
            ...
            BOOST_CHECK(CheckInputs(tx, state, pcashsTip.get(), true, test_flags, true, add_to_cache, txdata, &scriptchecks));
...
BOOST_FIXTURE_TEST_CASE(checkinputs_test, TestChain100Setup)
{
    ...
    CScript p2pk_scriptPubKey = CScript() << ToByteVector(cashbaseKey.GetPubKey()) << OP_CHECKSIG;
    CScript p2sh_scriptPubKey = GetScriptForDestination(CScriptID(p2pk_scriptPubKey));
    CScript p2pkh_scriptPubKey = GetScriptForDestination(cashbaseKey.GetPubKey().GetID());
    CScript p2wpkh_scriptPubKey = GetScriptForWitness(p2pkh_scriptPubKey);

    CBasicKeyStore keystore;
    keystore.AddKey(cashbaseKey);
    ...
    spend_tx.vin[0].prevout.hash = cashbaseTxns[0].GetHash();
    ...
    spend_tx.vout[2].scriptPubKey = CScript() << OP_CHECKLOCKTIMEVERIFY << OP_DROP << ToByteVector(cashbaseKey.GetPubKey()) << OP_CHECKSIG;
    spend_tx.vout[3].nValue = 11*CENT;
    spend_tx.vout[3].scriptPubKey = CScript() << OP_CHECKSEQUENCEVERIFY << OP_DROP << ToByteVector(cashbaseKey.GetPubKey()) << OP_CHECKSIG;
    ...
    // spend_tx is invalid according to DERSIG
    {
        ...
        BOOST_CHECK(!CheckInputs(spend_tx, state, pcashTip.get(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, nullptr));

        // If we call again asking for scriptchecks (as happens in
        // ConnectBlock), we should add a script check object for this -- we're
        // not caching invalidity (if that changes, delete this test case).
        std::vector<CScriptCheck> scriptchecks;
        BOOST_CHECK(CheckInputs(spend_tx, state, pcashTip.get(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_DERSIG, true, true, ptd_spend_tx, &scriptchecks));
    ...
    BOOST_CHECK(chainActive.Tip()->GetBlockHash() == block.GetHash());
    BOOST_CHECK(pcashTip->GetBestBlock() == block.GetHash());
    ...
    // Test CHECKLOCKTIMEVERIFY
    {
        ...
        BOOST_CHECK(CheckInputs(invalid_with_cltv_tx, state, pcashTip.get(), true, SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY, true, true, txdata, nullptr));
    }
    ...
    // TEST CHECKSEQUENCEVERIFY
    {
        ...
        BOOST_CHECK(CheckInputs(invalid_with_csv_tx, state, pcashTip.get(), true, SCRIPT_VERIFY_CHECKSEQUENCEVERIFY, true, true, txdata, nullptr));
    }
...

    {
        ...
        BOOST_CHECK(!CheckInputs(tx, state, pcashTip.get(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, nullptr));

        std::vector<CScriptCheck> scriptchecks;
        // Make sure this transaction was not cached (ie because the first
        // input was valid)
        BOOST_CHECK(CheckInputs(tx, state, pcashTip.get(), true, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, true, true, txdata, &scriptchecks));
