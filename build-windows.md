...
Below are some notes on how to build the SalemCash for Windows.

The options known to work for building the SalemCash on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Trusty 14.04 is recommended
and is the platform used to build the SalemCash blockchain for the Windows release binaries.
...
for Ubuntu Xenial does not produce working executables for some of the Salemcash Core applications.
...
Note that for WSL the SalemCash source path MUST be somewhere in the default mount file system, for
example /usr/src/salemcash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\salemcash`, for example:

    make install DESTDIR=/mnt/c/workspace/salemcash
...
causes two of the SalemCash executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the SalemCash code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the SalemCash source code).
