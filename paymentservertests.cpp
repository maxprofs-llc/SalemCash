...
static SendCashRecipient handleRequest(PaymentServer* server, std::vector<unsigned char>& data)
{
    RecipientCatcher sigCatcher;
    QObject::connect(server, SIGNAL(receivedPaymentRequest(SendCashRecipient)),
        &sigCatcher, SLOT(getRecipient(SendCashRecipient)));
    ...
    QObject::disconnect(server, SIGNAL(receivedPaymentRequest(SendCashRecipient)),
        &sigCatcher, SLOT(getRecipient(SendCashRecipient)));
...
void PaymentServerTests::paymentServerTests()
{
    ...
    // Payment request with amount overflow (amount is set to 20000001 SCS):
...
void RecipientCatcher::getRecipient(const SendCashRecipient& r)
{
    recipient = r;
}
