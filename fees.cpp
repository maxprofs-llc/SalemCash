...
{
    CAmount fee_needed = GetMinimumFeeRate(cash_control, pool, estimator, feeCalc).GetFee(nTxBytes);
...
CFeeRate GetMinimumFeeRate(const CCashControl& cash_control, const CTxMemPool& pool, const CBlockPolicyEstimator& estimator, FeeCalculation *feeCalc)
{
    /* User control of how to calculate fee uses the following parameter precedence:
       1. cash_control.m_feerate
       2. cash_control.m_confirm_target
       3. payTxFee (user-set global variable)
       4. nTxConfirmTarget (user-set global variable)
       The first parameter that is set is used.
    */
    CFeeRate feerate_needed ;
    if (cash_control.m_feerate) { // 1.
        feerate_needed = *(cash_control.m_feerate);
        if (feeCalc) feeCalc->reason = FeeReason::PAYTXFEE;
        // Allow to override automatic min/max check over cash control instance
        if (cash_control.fOverrideFeeRate) return feerate_needed;
    }
    else if (!cash_control.m_confirm_target && ::payTxFee != CFeeRate(0)) { // 3. TODO: remove magic value of 0 for global payTxFee
        feerate_needed = ::payTxFee;
        if (feeCalc) feeCalc->reason = FeeReason::PAYTXFEE;
    }
    else { // 2. or 4.
        // We will use smart fee estimation
        unsigned int target = cash_control.m_confirm_target ? *cash_control.m_confirm_target : ::nTxConfirmTarget;
        // By default estimates are economical iff we are signaling opt-in-RBF
        bool conservative_estimate = !cash_control.signalRbf;
        // Allow to override the default fee estimate mode over the CashControl instance
        if (cash_control.m_fee_mode == FeeEstimateMode::CONSERVATIVE) conservative_estimate = true;
        else if (cash_control.m_fee_mode == FeeEstimateMode::ECONOMICAL) conservative_estimate = false;
