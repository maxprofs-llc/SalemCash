...
const int SALEMCASH_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString salemcash_IPC_PREFIX("salemcash:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/salemcash-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/salemcash-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/salemcash-paymentrequest";
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the salemcash: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(SALEMCASH_IPC_PREFIX, Qt::CaseInsensitive)) // salemcash: URI
        {
            savedPaymentRequests.append(arg);

            SendCashRecipient r;
            if (GUIUtil::parseSalemcashURI(arg, &r) && !r.address.isEmpty())
            {
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    for (const QString& r : savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SALEMCASH_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = nullptr;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SALEMCASH_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click salemcash: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start salemcash: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling salemcash: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in salemcash: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("salemcash://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'salemcash://' is not a valid URI. Use 'salemcash:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(SALEMCASH_IPC_PREFIX, Qt::CaseInsensitive)) // salemcash: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseSalemcashURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Salemcash address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCashRecipient& recipient)
{
    if (!optionsModel)
        return false;
    ...
    if (request.IsInitialized()) {
        ...
        // This is re-checked just before sending cash in WalletModel::sendCash().
    ...
    for (const std::pair<CScript, CAmount>& sendingTo : sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Unauthenticated payment requests to custom salemcash addresses are not supported
        ...
        // Salemcash amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (IsDust(txOut, ::dustRelayFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(salemcashUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, const SendCashRecipient& recipient, QByteArray transaction)
{
    ...
    if (wallet->GetKeyFromPool(newKey)) {
        ...
        // This should never happen, because sending cash should have
