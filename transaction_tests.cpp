...
BOOST_AUTO_TEST_CASE(test_Get)
{
    ...
    BOOST_CHECK(AreInputsStandard(t1, cash));
    BOOST_CHECK_EQUAL(cash.GetValueIn(t1), (50+21+22)*CENT);
}
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        Cash cash;
        cash.nHeight = 1;
        cash.fCashBase = false;
        cash.out.nValue = 1000;
        cash.out.scriptPubKey = scriptPubKey;
        cash.emplace_back(std::move(cash));
    }

    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        std::vector<CScriptCheck> vChecks;
        CScriptCheck check(cash[tx.vin[i].prevout.n].out, tx, i, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, false, &txdata);
